...
Flag every claim whose recorded pane is absent, exited, or not working as a recovery candidate in the same tick — a claim with no live pane is never silently left in-progress. Two claimed or in-progress beads sharing one worktree path are a blocker for every owner (invariant 4); flag both, never pick one. Likewise, dependency-ready work auto-advances: recompute the ready frontier after each transition and dispatch/refill up to the admitted `maxWeightedSlots`/`maxWorkers` envelope without waiting for a human nudge.
...
The mission's routing plan — the table the user approved in the preview — is persisted as `routingPlan` in the mission-epic Beads metadata at bootstrap. Every dispatch must match it: same lane, same review policy. Reconcile each dispatch record against the approved plan; a dispatch whose lane or review policy differs with no recorded `routingJudgment` override is a violation — stop the lane and re-preview the contract. A change to a unit's risk lane or review policy is a material contract change (new preview + fresh approval); a harness override within an approved lane is recorded as `routingJudgment` in the dispatch record.
...
Claim through Beads before opening a dedicated Herdr workspace. Record `worktree`, `branch`, `base_sha`, `role`, `route`, `resource_class`, and `ownership`. Then dispatch through the canonical launcher: `scripts/dispatch_worker.py` for headless hermes lanes, or `scripts/dispatch_worker.py --harness omp|droid --worker-pane <lane pane>` for visible TUI lanes — the TUI mode runs the harness in the pane, injects the file-backed brief, verifies the agent is live within 30 seconds (working status or harness PID proven in `/proc`; a pane creation alone is never a dispatch), and attaches `watch_worker_completion.py` pane-aware. Every active worker has a verified completion-wake handle; a plain `herdr wait agent-status ... done` is telemetry, not a qualified watcher. Pane discipline, watcher qualification, and recovery: `references/visible-worker-lanes.md`; launcher contract: `references/worker-launch.md`. Persist its `beadsMetadata` only after the returned launcher and watcher identities are live and exact. Never return idle while an active worker is unwatched. `delegate_task` is the routine lane only: a separate headless Hermes sub-agent is acceptable for routine work, but never for standard/critical mission worker or reviewer roles, and never a substitute for the conductor's own session. Never use `spawn_agent --background`; standard and critical lanes dispatch through visible Herdr panes (invariant 11), and a routine lane that needs a watcher uses `dispatch_worker.py`.
...
Critical work always receives an independent review against the named integration branch. Standard work receives focused checks; an independent review is required only when the conductor's judgment demands it — shared seams, external effects, security-adjacent surfaces, or repeated correction — and the choice is recorded in the dispatch record. A plan-only lane is never independently reviewed: the planning harness's embedded critic is the plan review, and the frozen plan goes straight to implementation. The implementer fixes actionable findings; material fixes require fresh independent re-review. A reviewer that edits a finding cannot certify its own correction. **Review handoff is immediate:** when a visible Droid review reaches an idle/done verdict, inspect its final output in that same turn. Treat an idle pane whose transcript contains a verdict and focused-check recap as completed review evidence—not as an active worker, a watcher problem, or a reason to wait for another wake. For actionable findings, immediately send Droid the bounded fix brief, inspect the composer, submit it (verify the prompt did not remain staged), and confirm `working`; do not merely report the findings, promise a future integration, or answer wake guards with status narration. For PASS, immediately advance to the authorized Droid commit/merge lane after the declared evidence checks. After a fix PASS, make the serialized integration decision in the same turn; do not leave a reviewed candidate parked behind a stale Beads claim. Track correction count separately from dispatch/recovery retries. A reviewed PASS implementation lane is complete according to its plan state; do not mark it blocked merely because a downstream serial composition or integration dependency is not ready—the dependency graph must represent that wait.
...
**Integration guardrail false positives:** if the authorized Droid commit is rejected by its own safety/secret guard because a required test fixture contains a runtime-constructed sentinel, first inspect and preserve the exact finding. Do not silently bypass, disable the guard, weaken the fixture, or have the conductor commit. Record a blocked Bead with the exact file/line, guard message, why the fixture is required, and the explicit user choices (Droid external/manual integration, temporary guard disable, or fixture redesign). This is a real named human authority boundary; it is not the invented commit-authority gate prohibited elsewhere. Once the user chooses an in-envelope path, immediately unblock, steer Droid, verify the commit/merge evidence, and continue the merge queue in the same turn. **Do not reinterpret a TUI option such as “you run the commit yourself” as permission for the Conductor process to commit.** Invariant 2 still applies: obtain an explicit named external operator or a Droid-supported guard-approved path; otherwise retain the blocked boundary. Record the selected operator/path and exact resulting candidate and merge SHAs.
...
|---|---|---|---|
| 1 · transition sweep | after every material transition (claim, verdict, merge, recovery, restart) | fast consistency sweep + classification audit: every in-progress bead maps to a live pane + worktree, every live worker maps back to one bead, dispatch records complete, no claim without a live pane, no two claims share a worktree — and every classification carries its positive signal: dispatched ⇒ verified launch, blocked ⇒ durable boundary, in_progress ⇒ qualified watcher, idle controller ⇒ watchdog bound to the live session, paused ⇒ recorded reason | `kanban-orchestrator` |
...
25. **Wake-message narration loop.** Do not answer repeated wake guards with stale status prose. On the first wake after a material worker transition, inspect the pane transcript immediately; an idle reviewer verdict must advance in that same turn to a bounded Droid fix or authorized integration prompt. If a real recorded human boundary remains, state the exact decision once, retain the blocked state, and avoid reclassifying it or inventing new gates on subsequent identical wakes. Persist a compact `wake_audit_state` fingerprint (Bead status, boundary text, integration SHA, relevant pane state, and audit timestamp) with that first report. A wake-guard delivery is a liveness signal, not a new user request: if the next delivery has the same fingerprint, it is a strict no-op—do not emit a chat response, refresh heartbeats, re-run the audit, or repeat the boundary. Resume visible reporting only when a durable authority source materially changes (Beads/Git/Herdr/artifact/process), or when the named boundary is resolved.
